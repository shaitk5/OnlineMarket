...
public abstract class XmlReader {
    ...
    public static IMarketEngine readFile(Path path) throws JAXBException, IOException, DuplicateException, ValueOutOfRangeException {
        Market newMarket = new Market();
        testAndLoad(path, newMarket);
        newMarket.setFilePath(path);
        return newMarket;
    }
    ...
    private static Map<Integer, Store> getStoresMap(SuperDuperMarketDescriptor market){
        return market.getSDMStores()
                .getSDMStore()
                .stream()
                .collect(Collectors.toMap(SDMStore::getId,store -> new Store(store.getName(), store.getId(), store.getDeliveryPpk(), store.getLocation().getX(),store.getLocation().getY())));
    }
    ...
    private static Map<Integer, Product> getProductsMap(SuperDuperMarketDescriptor market){
        return market.getSDMItems()
                ...
                .toMap(SDMItem::getId,item -> new Product(item.getName(), item.getId(),
                        item.getPurchaseCategory().equals("Weight") ? Pricing.WEIGHT : Pricing.PRODUCT, 0)));
    }

    public static void testAndLoad(Path path, Market marketReceiving) throws JAXBException, IOException, DuplicateException, ValueOutOfRangeException {
        InputStream inputStream = new FileInputStream(new File(path.toString()));
        SuperDuperMarketDescriptor market = deserializeFrom(inputStream);

        Map<Integer, Integer> storesIdToOccurrences = market.getSDMStores().getSDMStore().stream()
                .collect(Collectors.toMap(SDMStore::getId, v -> 1, Integer::sum));
        Map<Integer, Integer> itemsIdToOccurrences = market.getSDMItems().getSDMItem().stream()
        ...
        if(checkIdShows(storesIdToOccurrences)){
            if(checkIdShows(itemsIdToOccurrences)){
                checkStoresItems(itemsIdToOccurrences, market.getSDMStores().getSDMStore());
            }
            else{
        ...
        readAllData(market, marketReceiving);
    }
    ...
    private static boolean checkIdShows(Map<Integer, Integer> map){
